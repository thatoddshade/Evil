...
class MapManager:
    ...
    def __init__(self, screen, player):
        ...
        self.register_map("anoying_room", portals=[
            ...
            NPC("wassa", nb_points=4, dialog=["(o_o)", "Is this dog eating un skeleton ?"])
        ])
        self.register_map("beach", portals=[
            ...
            NPC("faster", nb_points=6, dialog=["Want to run faster ?", "Hold Shift while moving to go fast!"])
        ])
        ...
        self.register_map("snowy_forest", portals=[
            ...
            NPC("allein", nb_points=6, dialog=["What's up ?", "Here, I don't see many people.", 'I\'m Allein. Welcome !'])
        ])
        ...
        self.register_map("cave", portals=[
            ...
            NPC("aiko", nb_points=5, dialog=["Hello ! My name is Aiko.", "It's a weird place, isn't it ? But I stay here.", "Ẃĥʸ ¡ß ٢ʜè®é à çǶéšť ?"])
        ])
    ...
    def register_map(self, name, portals=[], npcs=[]):
        # load the map
        print(f'Loading {name}.tmx...')
        tmx_data = pytmx.util_pygame.load_pygame(f"../map/{name}.tmx")
        map_data= pyscroll.data.TiledMapData(tmx_data)
        map_layer = pyscroll.orthographic.BufferedRenderer(map_data, self.screen.get_size())
        map_layer.zoom = 2

        # set a list to store the rectangles of collisions
        walls = []
